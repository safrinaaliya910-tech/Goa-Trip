...
export default function MembershipPage() {
  ...
  // Translatable static structural data arrays
  const benefits = [
    {
  ...
  return (
    <main className="min-h-screen bg-background">
            ...
            {membershipPower.map((card, index) => (
              <motion.div
                ...
                className={`relative border bg-card/80 p-8 backdrop-blur-md transition-all duration-500 ${
                  card.name.includes("Diamond") || card.name.includes("डायमंड") || card.name.includes("டயமண்ட்")
      ...
      <section className="px-6 py-24 md:py-32">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-16 text-center">
            <span className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.steps.sectionTitle")}</span>
            <h2 className="mt-4 text-3xl font-light text-foreground md:text-4xl">{t("membership.steps.heading")}</h2>
          </motion.div>
          ...
          <div className="grid gap-8 md:grid-cols-4">
            {steps.map((step, index) => (
              ...
              >
                ...
                <h3 className="mt-2 text-xl font-medium uppercase tracking-wider text-foreground">{step.title}</h3>
      ...
      <section className="bg-secondary/30 px-6 py-24 md:py-32">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mb-16 text-center">
            <span className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.plans.sectionTitle")}</span>
            <h2 className="mt-4 text-3xl font-light text-foreground md:text-4xl">{t("membership.plans.heading")}</h2>
          </motion.div>
            ...
            {tiers.map((tier, index) => (
              <motion.div
                ...
                className={`relative border bg-card p-8 transition-all duration-500 ${
                  tier.popular ? "border-primary shadow-[0_0_40px_rgba(212,175,55,0.15)]" : "border-border hover:border-primary/50"
                }`}
              >
                {tier.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                ...
                <div className="mb-6 flex items-center justify-center">
                  <tier.icon className={`h-12 w-12 ${tier.popular ? "text-primary" : "text-muted-foreground"}`} />
                </div>

                <h3 className="text-center text-2xl font-medium uppercase tracking-wider text-foreground">{tier.name}</h3>
                <p className="mt-4 text-center text-sm leading-relaxed text-muted-foreground">{tier.tagline}</p>

                <div className="mt-6 text-center">
                  <span className="text-sm text-muted-foreground">{t("membership.plans.priceLabel")}</span>
                  <div className="mt-2"><span className="text-4xl font-light text-primary">${tier.price}</span></div>
                </div>

                <p className="mt-6 border-t border-border pt-6 text-sm leading-relaxed text-muted-foreground">{tier.idealFor}</p>
                ...
                <button
                  ...
                  className={`mt-8 block w-full py-4 text-center text-sm uppercase tracking-widest transition-all duration-300 ${
                    tier.popular ? "bg-primary text-primary-foreground hover:bg-primary/90" : "border border-border text-foreground hover:border-primary hover:text-primary"
                  }`}
                >
            ...
            <div className="flex min-h-full items-start justify-center md:items-center">
              ...
              >
                ...
                <div className="border-b border-border p-6 md:p-8">
                  <p className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.checkout.secureTitle")}</p>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-widest">
                    <span className="rounded-full bg-primary px-3 py-1 text-primary-foreground">1. {t("membership.checkout.indicator1")}</span>
                    <span className={`rounded-full px-3 py-1 ${checkoutStep >= 2 ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"}`}>
                      2. {t("membership.checkout.indicator2")}
                    </span>
                    <span className="rounded-full border border-border px-3 py-1 text-muted-foreground">3. {t("membership.checkout.indicator3")}</span>
                    <span className="rounded-full border border-border px-3 py-1 text-muted-foreground">4. {t("membership.checkout.indicator4")}</span>
                  </div>
                </div>
                    ...
                    <div className="grid gap-8 md:grid-cols-[1.05fr_0.95fr]">
                      <div>
                        <div className="border border-border bg-background/40 p-5">
                          <p className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.checkout.selectedTitle")}</p>
                          <h4 className="mt-3 text-2xl font-light text-foreground">{selectedTier.name}</h4>
                          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{selectedTier.tagline}</p>
                          ...
                          <div className="mt-6 space-y-2 text-sm text-muted-foreground">
                            ...
                            <div className="flex items-center justify-between border-t border-border pt-3">
                              <span className="font-medium text-foreground">{t("membership.checkout.totalLabel")}</span>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.firstName")}</label>
                            <input
                              value={firstName}
                          ...
                          <div>
                            <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.lastName")}</label>
                            <input
                              value={lastName}
                        ...
                        <div>
                          <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.email")}</label>
                          <input
                            type="email"
                            value={customerEmail}
                        ...
                        <div>
                          <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.phone")}</label>
                          <input
                            type="tel"
                            value={customerPhone}
                        ...
                        <div>
                          <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.address")}</label>
                          <input
                        ...
                        <div>
                          <label className="mb-2 block text-sm text-foreground">{t("membership.checkout.form.city")}</label>
                          <input
                            value={customerCity}
                ...
                {checkoutStep === 2 && (
                  <div className="p-6 md:p-8">
                    <div className="grid gap-8 md:grid-cols-[1fr_0.95fr]">
                      <div className="border border-border bg-background/40 p-5">
                        <p className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.checkout.summary.title")}</p>
                        <div className="mt-5 space-y-3 text-sm text-muted-foreground">
                          ...
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span className="text-foreground truncate max-w-[60%] text-right">{customerAddress}</span>
                          </div>
                          <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center justify-between border-t border-border pt-3">
                            <span className="font-medium text-foreground">{t("membership.checkout.summary.amount")}</span>
                            <span className="text-xl font-light text-primary">${selectedTier.price}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="border border-primary/20 bg-gradient-to-b from-primary/5 to-transparent p-5">
                        <p className="text-xs uppercase tracking-[0.3em] text-primary">{t("membership.checkout.confirm.title")}</p>
                        <h4 className="mt-3 text-xl font-light text-foreground">{t("membership.checkout.confirm.heading")}</h4>
                        <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{t("membership.checkout.confirm.desc")}</p>

                        <div className="mt-5 rounded-sm border border-primary/20 bg-background/30 p-4">
                          <div className="flex items-center gap-3">
                            <Sparkles className="h-5 w-5 text-primary" />
                            <p className="text-sm text-foreground">{t("membership.checkout.confirm.sparkNote")}</p>
                          </div>
                        </div>
